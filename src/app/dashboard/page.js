...
export default function Dashboard() {
  ...
  if (password !== process.env.NEXT_PUBLIC_ADMIN_PASSWORD) {
    ...
    const sameLoc = (x, y) => { return x.latitude == y.latitude && x.longitude == y.longitude && x.range == y.range && x.operatorId == y.operatorId && x.active == y.active }
    ...
    return (
      <div className="grid items-center justify-items-center min-h-screen place-content-center font-[family-name:var(--font-geist-sans)]">
        ...
        <div className="text-center" id="Locations">
              ...
              <tbody id="locations" className="bg-gray-50 divide-y divide-green-500">
                {locations.map((location, index) => (
                  <tr key={index} className="hover:bg-gray-100 transition-all duration-300">
                    ...
                    <td>
                      <select className='text-center text-xl text-gray-800 bg-amber-400 rounded-2xl border-2 border-amber-800 hover:bg-amber-500 hover:text-gray-950 hover:border-amber-700 transition-all duration-300' id={`loc${location.id}_op`} defaultValue={location.operatorId}>
                    ...
                    <td><button className="font-bold text-center p-2 bg-red-700 rounded-2xl m-2 cursor-pointer border-2 border-red-900 transition-all duration-300 hover:bg-red-800 hover:border-red-950" onClick={() => {
          ...
          <button type='button' className="text-2xl font-bold text-center p-3 text-gray-950 bg-cyan-400 rounded-2xl m-2 cursor-pointer transition-all duration-300 hover:bg-cyan-500 hover:text-gray-800" onClick={() => window.location.href = "/mapboard?password=admin123"}><Image src='/marker.svg' width={32} height={32} alt='marker' />Manual Location</button>
          ...
          <button type='button' className="text-2xl font-bold text-center p-3 text-white bg-purple-600 rounded-2xl m-2 cursor-pointer transition-all duration-300 hover:bg-purple-700 hover:text-gray-300" onClick={() => require("downloadjs")(JSON.stringify(locations), `locations_${Date.now()}.json`, "text/plain")}><Image src='/folder.svg' width={32} height={32} alt='folder' />Create Backup</button>
        </div>
        <div className="text-center" id="Operators">
          <h2 className="text-2xl font-semibold text-center box-border p-3">Antenna Operators</h2>
          <Suspense>
            <table className='overflow-auto'>
              ...
              <tbody className='bg-gray-50 divide-y divide-green-500'>
                {operators.map((operator, index) => (
                  <tr key={index} className="hover:bg-gray-100 transition-all duration-300">
                    <td><label className='text-center text-base text-gray-900'>{operator.id}</label></td>
                    ...
                    <td><button type='submit' className="font-bold text-center p-2 bg-red-700 rounded-2xl m-2 cursor-pointer border-2 border-red-900 transition-all duration-300 hover:bg-red-800 hover:border-red-950" onClick={(event) => {
                      ...
                      }).then(response => console.log("Operator deleted:", response)).catch(error => console.error('Error deleting operator:', error));
                    }}><Image src='/trash.svg' width={20} height={20} alt='trash' /></button></td>
                  </tr>
                ))}
              </tbody>
          ...
          <button type='button' className="text-2xl font-bold text-center p-3 text-white bg-teal-600 rounded-2xl m-2 cursor-pointer transition-all duration-300 hover:bg-teal-700 hover:text-gray-300" onClick={() => {
            const newOperator = {
              name: "Unnamed",
              origin: "Unknown",
            }
          ...
          <button type='submit' className="text-2xl font-bold text-center p-3 text-white bg-rose-500 rounded-2xl m-2 cursor-pointer transition-all duration-300 hover:bg-rose-600 hover:text-gray-300" onClick={() => {
          ...
          <button type='button' className="text-2xl font-bold text-center p-3 text-white bg-emerald-400 rounded-2xl m-2 cursor-pointer transition-all duration-300 hover:bg-emerald-500 hover:text-gray-300" onClick={() => require("downloadjs")(JSON.stringify(operators), `operators_${Date.now()}.json`, "text/plain")}><Image src='/backup.svg' width={32} height={32} alt='backup' />Create Backup</button>
        </div>
        <div className='flex flex-col items-center' id="ChangePassword">
          <h2 className="text-2xl font-semibold text-center box-border p-3">Login</h2>
          ...
          <button type='button' className="text-2xl font-bold text-center p-3 text-white bg-blue-800 rounded-2xl m-2 cursor-pointer transition-all duration-300 hover:bg-blue-950 hover:text-gray-300" onClick={(event) => {
            ...
            // process.env.NEXT_PUBLIC_ADMIN_PASSWORD=pass1
            alert("Password changed succesfully")
          }}><Image src='/password.svg' width={32} height={32} alt='password' />Change Password</button>
        </div>
        <button type='button' className="text-2xl font-bold text-center p-3 text-black bg-yellow-300 rounded-2xl m-2 cursor-pointer transition-all duration-300 hover:bg-yellow-500 hover:text-gray-800" onClick={() => window.location.href = "/"}><Image src='/home.svg' width={32} height={32} alt='home' />Return Home</button>
      </div>
    );
  }
}
